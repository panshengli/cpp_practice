...
　　　　}
  ...
  3. 多态
     - 多态的实现主要分为静态多态和动态多态
       - 静态多态: 针对泛型编程，主要是**重载**，在编译的时候就已经确定
       - 动态多态: 是用虚函数机制实现的，在运行期间动态绑定,派生类**重写**
  - [2021.01.11]
  1. 关于多线程问题
     - look_guard:
  ...
  - [2021.01.12]
  1. const
     - 在函数后面添加const, const是修饰this指向的对象的
     - 成员函数不会修改类的数据, 否则会编译报错(现象)
  2. 空类占用的空间以及多重继承空类占用的空间
     - 空类、单一继承及多重继承占用一个空间
     - 虚继承涉及虚表(虚指针)占用空间为sizeof = 4